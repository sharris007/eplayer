...
export class Book extends Component {
  ...
  annotationCallBack = (eventType, data) => {
    ...
    console.log('ann call back data', data);
  ...
  render() {
   ...
   const callbacks = {};
    ...
    const arr = this.props.book.playlist;
   
   this.state.pageDetails.playListURL = this.props.book.playlist.content;
      if(arr.length != 0){
        this.state.pageDetails.baseUrl     = this.props.book.playlist.baseUrl;
        this.state.pageDetails.currentPageURL      = this.props.book.playlist.content[1];
        this.state.pageDetails.playListURL = this.props.book.playlist.content;
        console.log("this.state.pageDetails" , this.state.pageDetails);        
        // this.setState({
        //   currentPageDetails: this.props.book.playlist.content[1]
        // }); 
      }
    ...
    return (
      <div>
        ...
        />
          ...
          <div >
          { arr.length != 0 ? <PageViewer src={this.state.pageDetails} sendPageDetails={this.onPageChange} /> : ' '}
            
            {arr.length != 0 ? <Annotation annotationData={annotionData} contentId="pxe-viewer" annotationEventHandler={this.annotationCallBack.bind(this)} currentPageDetails={this.state.currentPageDetails} /> : ''}
          </div>
